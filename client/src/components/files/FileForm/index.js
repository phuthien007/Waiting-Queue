...
const FileForm = ({ type, data, saveData, loadingSave, isSuccess }) => {
        ...
        >
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Tên file không được bỏ trống",
              },
              {
                validator: (_, value) =>
                  !value || (value.length >= 0 && value.length <= 256)
                    ? Promise.resolve()
                    : Promise.reject(
                        new Error("Tên file chỉ chứa tối đa 256 kí tự")
                      ),
              },
            ]}
          >
            <Input type="text" placeholder="Tên file" />
          </Form.Item>
