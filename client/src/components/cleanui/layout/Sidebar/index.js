...
import { changeSettings, selectSettings, setPrimaryColor } from "store/settingSlice";
...
const Sidebar = () => {
  ...
  const resetColor = () => {
    dispatch(setPrimaryColor({ color: defaultColor }));
  };

  const changeSetting = (setting, value) => {
    dispatch(changeSettings({ setting, value }));
  };

  const toggleSettings = (e) => {
    e.preventDefault();
    dispatch(
      changeSettings({ setting: "isSidebarOpen", value: !isSidebarOpen })
    );
  };

  const selectMenuLayoutType = (e) => {
    const { value } = e.target;
    dispatch(changeSettings({ setting: "menuLayoutType", value }));
  };
  ...
  const selectRouterAnimation = (value) => {
    dispatch(changeSettings({ setting: "routerAnimation", value }));
  };

  const selectLocale = (value) => {
    dispatch(changeSettings({ setting: "locale", value }));
  };
          ...
          <div className={style.cui__sidebar__inner}>
            ...
            <div>
              ...
              <div className={style.cui__sidebar__item}>
                <div className={style.cui__sidebar__label}>
                  Left Menu: Collapsed
                </div>
