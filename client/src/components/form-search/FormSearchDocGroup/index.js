...
const FormSearchDocGroup = ({ setDataSearch }) => {
  const [form] = Form.useForm();
  const handleSearch = (values) => {
    values.searchDocGroup = values.searchDocGroup?.trim();
  ...
  return (
    <Form onFinish={handleSearch} form={form} layout="inline">
      <Row justify="start">
        <Col>
          <Form.Item label="Tìm kiếm" name="searchDocGroup">
            <Input placeholder="Từ khóa" allowClear />
          </Form.Item>
        </Col>
        <Col>
          <Form.Item name="departmentDocGroup">
            <Input placeholder="Phòng quản lý" allowClear />
          </Form.Item>
        </Col>
        <Col>
          <Form.Item name="statusDocGroup">
...
export default FormSearchDocGroup;
